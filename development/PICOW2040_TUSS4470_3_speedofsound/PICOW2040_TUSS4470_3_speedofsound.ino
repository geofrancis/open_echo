...
int SPEED_OF_SOUND = 330;     // Speed of sound in m/s (330=air, ~1500=water)
...
float calculateSpeedOfSound(float T, float S, float D) {
  float speed = 1449.2 + (4.6 * T)
                - (0.055 * T * T)
                + (0.00029 * T * T * T)
                + (1.34 - 0.010 * T) * (S - 35.0)
                + (0.016 * D);
...
float ecToSalinity(float mS_cm, float tempC) {
  // Temperature compensation (standardized to 25C)
  float ec25 = mS_cm / (1.0 + 0.019 * (tempC - 25.0));
  float salinity = ec25 * 0.66;
  return salinity;
}
...
void Data() {
  ...
  float mS_cm = voltage * 1.0;

  float salinity = ecToSalinity(mS_cm, tempC);

  // 4. Calculate Speed of Sound (Assuming depth = 1 meter for surface)
  float depths = 1.0;
  float speed = calculateSpeedOfSound(tempC, salinity, depths);
...
void send_mavlink_depth() {
  ...
  mavlink_message_t msg;
  uint8_t buf[MAVLINK_MAX_PACKET_LEN];
  mavlink_msg_distance_sensor_pack(sysid, compid, &msg, time_boot_ms, 300, 1200, Distance1, type, id, orientation, covariance, horizontal_fov, vertical_fov, signal_quality, quaternion);
  uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);
  Serial2.write(buf, len);
}
...
void send_NMEA_depth() {
  ...
  Serial3.print(fullSentence);
}



void PWM_depth_output() {
  int Distance = (depth_m * 100);
